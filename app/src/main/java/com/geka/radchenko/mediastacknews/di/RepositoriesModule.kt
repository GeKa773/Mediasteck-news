...
object RepositoriesModule {
    @Provides
    @Singleton
    fun provideRepository(api: Api, dataBase: DataBase, preferencesManager: PreferencesManager): Repository {
        return Repository(api, dataBase, preferencesManager)
    }
    @Provides
    @Singleton
    fun provideDatabase(application: Application): DataBase {
        return Room.databaseBuilder(application, DataBase::class.java, DATABASE_NAME).fallbackToDestructiveMigration().build()
    }
